...
export default function Diagnosis() {
    ...
    runDiagnosis.mutate(
      { data: form },
      {
        onSuccess: (data) => {
          ...
          setResult(r);
          setSessionTitle(form.goal.substring(0, 60));
          setStep("result");
        ...
        <PageHeader
          ...
          right={
            <div className="flex items-center gap-2">
              <HudBtn variant="ghost" onClick={() => printDiagnosis(result, sessionTitle)} data-testid="button-export-pdf">
                Export PDF
              </HudBtn>
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "var(--sos-bg)" }}>
      <PageHeader
        title="Strategic Diagnosis"
        sub="Describe your position. Receive a structured analysis with ranked priorities and a clear next step."
      />
        ...
        <div className="px-8 py-7 max-w-2xl space-y-7">
          <div className="grid grid-cols-2 gap-6">
            <FieldGroup label="Current Goal *">
              <input
                id="goal" value={form.goal} onChange={(e) => setForm({ ...form, goal: e.target.value })}
                placeholder="State the specific outcome you are working toward"
            ...
            <FieldGroup label="Industry / Domain *">
              <input
                id="industry" value={form.industry} onChange={(e) => setForm({ ...form, industry: e.target.value })}
                placeholder="e.g. AI Consulting, SaaS, Research"
          ...
          <FieldGroup label="Current Assets">
            <textarea
              id="assets" value={form.assets} onChange={(e) => setForm({ ...form, assets: e.target.value })}
              placeholder="Skills, credentials, tools, relationships, projects, existing revenue"
              rows={3} data-testid="input-assets"
          ...
          <FieldGroup label="Primary Bottleneck">
            <textarea
              id="bottleneck" value={form.bottleneck} onChange={(e) => setForm({ ...form, bottleneck: e.target.value })}
              placeholder="What is the single thing most limiting your progress right now?"
              rows={2} data-testid="input-bottleneck"
              style={{ width: "100%", fontSize: 13, paddingBottom: 8, paddingTop: 4, resize: "none", lineHeight: 1.6 }}
            />
          </FieldGroup>
          ...
          <div className="grid grid-cols-2 gap-6">
            <FieldGroup label="Constraints">
              <textarea
                id="constraints" value={form.constraints} onChange={(e) => setForm({ ...form, constraints: e.target.value })}
                placeholder="Time per week, budget, access, competing priorities"
                rows={2} data-testid="input-constraints"
                style={{ width: "100%", fontSize: 13, paddingBottom: 8, paddingTop: 4, resize: "none", lineHeight: 1.6 }}
              />
            </FieldGroup>
            <FieldGroup label="Timeline">
              <input
                id="deadline" value={form.deadline} onChange={(e) => setForm({ ...form, deadline: e.target.value })}
                placeholder="e.g. 60 days, end of Q3"
          ...
          <FieldGroup label="Definition of Success">
            <textarea
              id="desiredOutcome" value={form.desiredOutcome} onChange={(e) => setForm({ ...form, desiredOutcome: e.target.value })}
              placeholder="What does a successful outcome look like in concrete terms?"
              rows={2} data-testid="input-desired-outcome"
              style={{ width: "100%", fontSize: 13, paddingBottom: 8, paddingTop: 4, resize: "none", lineHeight: 1.6 }}
            />
          </FieldGroup>
